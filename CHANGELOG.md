...
### Fixed
- Critical conflict: the Theme Settings dynamic CSS (custom colors/typography) output after global styles and permanently overrode every demo style variation once generated. Demo import and variation reset now clear the override; saving Theme Settings regenerates it (last action wins). Browser-verified against the Horizon and Nova palettes.
- Refreshed theme screenshot (1200x900, live NOVA homepage render).

### Verified
- Theme settings live test: save -> persist -> frontend effect (colors, container width, radii).
- Module toggles disable their CPTs (tested module_faqs).
- 10/10 demo imports with dynamic CSS clean, 0 dead anchors, 0 broken links.
